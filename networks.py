...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.entry_block = nn.Sequential(
            ...
            nn.utils.spectral_norm(
                nn.Conv2d(3, base_channels, kernel_size=7, bias=use_bias)),
        ...
        self.downscale_block = RescaleBlock(n_downsampling, 0.5, base_channels,
                                            True)
        ...
        for _ in range(n_blocks):
            ...
            bottleneck_block += [
                ResnetBlock(base_channels * 2**n_downsampling,
                            use_bias=use_bias)
            ]
        ...
        self.upscale_block = RescaleBlock(n_downsampling, 2.0, base_channels,
                                          True)

        # Final block
        # No stride so image dims are kept and channels dim shrinks to 3 (output image channels)
        self.final_block = nn.Sequential(
            nn.ReflectionPad2d(3), nn.Conv2d(base_channels, 3, kernel_size=7),
            nn.Tanh())
    ...
    def forward(self, input_tensor, output_size, random_affine):
        ...
        if random_affine is None:
            ...
            feature_map = self.geo_transform.forward(feature_map, output_size,
                                                     random_affine)

        # Downscale block
        feature_map, downscales = self.downscale_block.forward(
        ...
        # Upscale block
        feature_map, _ = self.upscale_block.forward(feature_map,
                                                    pyramid=downscales,
                                                    skip=self.skip)
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def make_net(self):
        ...
        net += [
            nn.utils.spectral_norm(
                nn.Conv2d(3, base_channels, kernel_size=3, stride=1)),
        ...
        net += nn.Sequential(
            nn.utils.spectral_norm(
                nn.Conv2d(base_channels * 2, 1, kernel_size=1)), nn.Sigmoid())

        # Make it a valid layers sequence and return
        return nn.Sequential(*net)

    def forward(self, input_tensor, scale_weights):
        aggregated_result_maps_from_all_scales = self.nets[0](
            input_tensor) * scale_weights[0]
        map_size = aggregated_result_maps_from_all_scales.shape[2:]

        # Run all nets over all scales and aggregate the interpolated results
        for net, scale_weight, i in zip(self.nets[1:], scale_weights[1:],
                                        list(range(1, len(scale_weights)))):
...
class RescaleBlock(nn.Module):
    def __init__(self, n_layers, scale=0.5, base_channels=64, use_bias=True):
        ...
        i_range = list(range(n_layers)) if scale < 1 else list(
            range(n_layers - 1, -1, -1))
            ...
            self.conv_layers[i] = nn.Sequential(
                ...
                nn.utils.spectral_norm(
                    nn.Conv2d(in_channels=base_channels *
                              2**(i + in_channel_power),
                              out_channels=base_channels *
                              2**(i + out_channel_power),
                              kernel_size=3,
                              stride=1,
                              bias=use_bias)),
    ...
    def forward(self,
        ...
        return (feature_map,
                all_scales) if return_all_scales else (feature_map, None)
...
class RandomCrop(nn.Module):
    ...
    def forward(self, input_tensors, crop_size=None):
        ...
        if crop_size is None:
            cr_v_sz, cr_h_sz = np.clip(self.crop_size, [0, 0],
                                       [im_v_sz - 1, im_h_sz - 1])
            cr_v_sz, cr_h_sz = np.uint32(
                np.floor(
                    np.array([cr_v_sz, cr_h_sz]) * 1.0 / self.must_divide) *
                self.must_divide)
        else:
        ...
        out_tensors = [
            input_tensor[:, :, top_left_v:top_left_v + cr_v_sz,
                         ...
                         cr_h_sz] if input_tensor is not None else None
            for input_tensor in input_tensors
        ]

        return (out_tensors, (top_left_v,
                              top_left_h)) if self.return_pos else out_tensors
...
class SwapCrops(nn.Module):
    ...
    def forward(self, input_tensor):
        cr_v_sz, cr_h_sz = np.uint32(
            np.random.rand(2) * (self.max_crop_size - self.min_crop_size) +
            self.min_crop_size)

        [crop_1], (top_left_v_1,
                   top_left_h_1) = self.rand_crop_1.forward([input_tensor],
                                                            (cr_v_sz, cr_h_sz))
        [crop_2], (top_left_v_2,
                   top_left_h_2) = self.rand_crop_1.forward([input_tensor],
                                                            (cr_v_sz, cr_h_sz))

        output_tensor = torch.zeros_like(input_tensor)
        output_tensor[:, :, :, :] = input_tensor

        output_tensor[:, :, top_left_v_1:top_left_v_1 + cr_v_sz,
                      top_left_h_1:top_left_h_1 + cr_h_sz] = crop_2
        output_tensor[:, :, top_left_v_2:top_left_v_2 + cr_v_sz,
                      top_left_h_2:top_left_h_2 + cr_h_sz] = crop_1
        ...
        loss_mask[:, :, top_left_v_1:top_left_v_1 + cr_v_sz,
                  top_left_h_1 - mw:top_left_h_1 + mw] = 0
        loss_mask[:, :, top_left_v_1 - mw:top_left_v_1 + mw,
                  top_left_h_1:top_left_h_1 + cr_h_sz] = 0
        loss_mask[:, :, top_left_v_1:top_left_v_1 + cr_v_sz,
                  top_left_h_1 + cr_h_sz - mw:top_left_h_1 + cr_h_sz + mw] = 0
        loss_mask[:, :,
                  top_left_v_1 + cr_v_sz - mw:top_left_v_1 + cr_v_sz + mw,
                  top_left_h_1:top_left_h_1 + cr_h_sz] = 0
        loss_mask[:, :, top_left_v_2:top_left_v_2 + cr_v_sz,
                  top_left_h_2 - mw:top_left_h_2 + mw] = 0
        loss_mask[:, :, top_left_v_2 - mw:top_left_v_2 + mw,
                  top_left_h_2:top_left_h_2 + cr_h_sz] = 0
        loss_mask[:, :, top_left_v_2:top_left_v_2 + cr_v_sz,
                  top_left_h_2 + cr_h_sz - mw:top_left_h_2 + cr_h_sz + mw] = 0
        loss_mask[:, :,
                  top_left_v_2 + cr_v_sz - mw:top_left_v_2 + cr_v_sz + mw,
                  top_left_h_2:top_left_h_2 + cr_h_sz] = 0
    ...
    def forward(self, input_tensor, target_size, shifts):
        ...
        pad = f.pad(input_tensor,
                    (np.abs(np.int(np.ceil(sz[3] * shifts[0]))),
                     np.abs(np.int(np.ceil(-sz[3] * shifts[1]))), 0, 0),
                    'reflect')
        target_size4d = torch.Size(
            [pad.shape[0], pad.shape[1], target_size[0], target_size[1]])

        grid = homography_grid(theta.expand(pad.shape[0], -1, -1),
                               target_size4d)
