...
def generate_one_scene(gan, input_tensor, scene_script, frame_shape, center):
    ...
    for i, (scale_v, scale_h, shift_l, shift_r) in enumerate(scene_script):
        output_image = generate_one_frame(gan, input_tensor, frame_shape,
                                          [scale_v, scale_h],
                                          [shift_l, shift_r], center)
...
def generate_full_video(video_script, frame_shape):
    ...
    for i, (nameses, scene_script_names,
            scene_script_params) in enumerate(video_script):
        ...
        scenes = []
        for names, scene_script_name, scene_script_param in zip(
                nameses, scene_script_names, scene_script_params):
            ...
            for name in names:
                ...
                conf.test_params_path = os.path.dirname(
                    os.path.abspath(__file__)) + INPUT_DICT[name][1]
                gan = InGAN(conf)
                gan.G.load_state_dict(
                    torch.load(conf.test_params_path,
                               map_location='cuda:0')['G'])
                ...
                if scene_script_param[1] is None:
                    cur_scene_script_param[
                        1] = cur_frame_shape[0] * 1.0 / input_tensor.shape[2]
                    print('max scale vertical:', cur_scene_script_param[1])
                if cur_scene_script_param[3] is None:
                    cur_scene_script_param[
                        3] = cur_frame_shape[1] * 1.0 / input_tensor.shape[3]
                    print('max scale horizontal:', cur_scene_script_param[3])

                scene_script = make_scene_script(scene_script_name,
                                                 *cur_scene_script_param)

                center = (cur_scene_script_param[4] is not None)

                scene = generate_one_scene(
                    gan, input_tensor, scene_script,
                    np.array([cur_frame_shape[0], cur_frame_shape[1]]), center)
...
def prepare_geometric(base_sz, scale, geo_shifts, device: torch.device = torch.device('cuda')):
    ...
    target_size4d = torch.Size(
        [in_mask.shape[0], in_mask.shape[1], out_size[0], out_size[1]])
    grid = homography_grid(theta.expand(in_mask.shape[0], -1, -1),
                           target_size4d)
    out_mask = f.grid_sample(in_mask,
                             grid,
                             mode='bilinear',
                             padding_mode='zeros')
