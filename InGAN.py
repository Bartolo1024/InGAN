...
class LRPolicy(object):
    ...
    def __call__(self, citer):
        return 1. - max(
            0.,
            float(citer - self.start) / float(self.end - self.start))
...
class InGAN:
    ...
    def test(self,
        ...
        with torch.no_grad():
            self.G_pred = self.G.forward(Variable(input_tensor.detach()),
            ...
            if run_d_pred:
                ...
                scale_weights_for_input = get_scale_weights(
                    ...
                    scale_factor=self.conf.D_scale_factor)
                self.D_preds = [
                    self.D.forward(Variable(input_tensor.detach()),
                                   scale_weights_for_input),
                    self.D.forward(Variable(self.G_pred.detach()),
                                   scale_weights_for_output)
                ]
            else:
                self.D_preds = None
    ...
    def train_g(self):
        ...
        self.input_tensor_noised = self.input_tensor + (
            torch.rand_like(self.input_tensor) - 0.5) * 2.0 / 255
        ...
        if self.conf.reconstruct_loss_stop_iter < self.cur_iter:
            ...
            self.loss_G = (self.conf.reconstruct_loss_proportion *
                           self.loss_G_reconstruct + self.loss_G_GAN)
        ...
        if self.cur_iter > self.conf.G_extra_inverse_train_start_iter:
            for _ in range(self.conf.G_extra_inverse_train):
                ...
                self.inverse = self.G.forward(
                    ...
                    output_size=self.input_tensor.shape[2:],
                    random_affine=-random_affine)
                self.loss_G_inverse = (self.criterionReconstruction(
    ...
    def train_d(self):
        ...
        real_example_with_noise = self.real_example + (
            torch.rand_like(self.real_example[-1]) - 0.5) * 2.0 / 255.0

        # Discriminator forward pass over real example
        self.d_pred_real = self.D.forward(real_example_with_noise,
                                          self.scale_weights)
        ...
        g_pred_with_noise = self.G_pred.detach() + (
            torch.rand_like(self.G_pred) - 0.5) * 2.0 / 255

        # Discriminator forward pass over generated example example
        self.d_pred_fake = self.D.forward(g_pred_with_noise,
                                          self.scale_weights)

        # Calculate discriminator loss
        self.loss_D_fake = self.criterionGAN(self.d_pred_fake,
                                             is_d_input_real=False)
        self.loss_D_real = self.criterionGAN(self.d_pred_real,
    ...
    def train_one_iter(self, cur_iter, input_tensors):
        ...
        for input_tensor in input_tensors:
            ...
            if np.random.rand() < self.conf.crop_swap_probability:
                swapped_input_tensor, loss_mask = self.SwapCrops.forward(
                    input_tensor)
                [input_crop, mask_crop
                 ] = self.RandCrop.forward([swapped_input_tensor, loss_mask])
        ...
        self.losses_G_gan[cur_iter %
                          self.conf.print_freq] = self.loss_G_GAN.item()
        self.losses_D_fake[cur_iter %
                           self.conf.print_freq] = self.loss_D_fake.item()
        self.losses_D_real[cur_iter %
                           self.conf.print_freq] = self.loss_D_real.item()
        if self.conf.reconstruct_loss_stop_iter > self.cur_iter:
            self.losses_G_reconstruct[
                cur_iter %
                self.conf.print_freq] = self.loss_G_reconstruct.item()
