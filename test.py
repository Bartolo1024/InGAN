...
def concat_images(images, margin, input_spot):
    ...
    for i in range(len(images)):
        for j in range(len(images)):
            ...
            if [i, j] == input_spot:
                collage[top_left_corner_h - margin // 2:bottom_right_corner_h +
                        margin // 2,
                        top_left_corner_w - margin // 2:bottom_right_corner_w +
                        margin // 2, :] = [255, 0, 0]
            collage[top_left_corner_h:bottom_right_corner_h,
                    top_left_corner_w:bottom_right_corner_w] = images[j][i]
...
def generate_images_for_collage(gan, input_tensor, scales, must_divide):
    ...
    output_images = [[[None] for _ in range(len(scales))]
    ...
    for i, scale_h in enumerate(scales):
        for j, scale_w in enumerate(scales):
            output_images[i][j] = test_one_scale(gan, input_tensor,
                                                 [scale_h, scale_w],
                                                 must_divide)
...
def retarget_video(gan, input_tensor, scales, must_divide, output_dir_path):
    ...
    for i, (scale_h, scale_w) in enumerate(scales):
        output_image = test_one_scale(gan, input_tensor, [scale_h, scale_w],
                                      must_divide)
        frames[i, 0:output_image.shape[0],
               0:output_image.shape[1], :] = output_image
    frame_size = frame_shape[:2]
    fourcc = cv2.VideoWriter_fourcc(*'MP4V')
    writer = cv2.VideoWriter(output_dir_path + '/vid.mp4', fourcc, 20.0,
                             frame_size)

    frames = frames.astype(np.uint8)[::-1]
...
def generate_collage_and_outputs(conf, gan, input_tensor):
    output_images = generate_images_for_collage(gan, input_tensor,
                                                conf.collage_scales,
                                                conf.must_divide)

    for i in range(len(output_images)):
        for j in range(len(output_images)):
            Image.fromarray(output_images[i][j],
                            'RGB').save(conf.output_dir_path +
                                        '/test_%d_%d.png' % (i, j))
    ...
    Image.fromarray(np.uint8(collage),
                    'RGB').save(conf.output_dir_path + '/test_collage.png')
...
def _make_homography_mask(in_mask, target_size, rand_h):
    ...
    target_size4d = torch.Size(
        [in_mask.shape[0], in_mask.shape[1], target_size[0], target_size[1]])
    grid = homography_grid(theta.expand(in_mask.shape[0], -1, -1),
                           target_size4d)
        ...
        for scale2 in conf.non_rect_scales:
            ...
            for shift1 in shift_range:
                for shift2 in shift_range:
                    ...
                    out_pad = np.uint8(255 * np.ones([
                        np.uint32(np.floor(sz[2] * scale[0])),
                    ...
                    out_size = (np.uint32(
                        ...
                        must_divide),
                                np.uint32(
                                    np.floor(scale[1] * in_size[1] * 1.0 /
                                             must_divide) * must_divide))

                    if pad_r > 0:
                        in_mask[:, :, :,
                                pad_l:-pad_r] = torch.ones_like(input_tensor)
                        input_for_regular[:, :, :, pad_l:-pad_r] = input_tensor
                    else:
                        in_mask[:, :, :,
                                pad_l:] = torch.ones_like(input_tensor)
                        input_for_regular[:, :, :, pad_l:] = input_tensor

                    out = test_one_scale(gan,
                                         input_tensor,
                                         out_size,
                                         conf.must_divide,
                                         affine=shifts,
                    ...
                    shift_str = "{1:0{0}d}_{3:0{2}d}".format(
                        2 if shift1 >= 0 else 3, int(10 * shift1),
                        2 if shift2 >= 0 else 3, int(10 * shift2))
                    ...
                    Image.fromarray(out, 'RGB').save(
                        ...
                        '/scale_%02d_%02d_transform %s_ingan.png' %
